...
polling_data <- data.table::fread('data/polling_mourakiboun.csv') %>% as.data.frame %>% as_tibble

# polling center metadata

metadata <- data.table::fread('data/pollingCenters.csv') %>% as.data.frame %>% as_tibble

# combined_data

combined <- data.table::fread("data/points.csv") %>% as.data.frame %>% as_tibble
...
combined %>% as.data.frame %>% leaflet() %>% addTiles() %>% addMarkers(lat=~latitude,lng=~longitude)

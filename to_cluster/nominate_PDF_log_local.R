...
constrain_pos <- c(which(sen$party==200))
#constrain_pos <- c(which(grepl('SESSIONS',row.names(sv.yay))))
#constrain_neg <- c(which(grepl('REID',row.names(sv.yay))))
constrain_neg <- NULL
...
Y <- c(sv.yay)
script_file <- "C:/Users/bobku/Box Sync/Dissertation/Field Research/ARP votes/nominate.h"

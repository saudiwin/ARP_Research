...
first_d_pos <- c('Chaker Ayadi','Imed Ouled Jebril')
...
# Some bills only have one votes, which may be screwing up the analysis
count_NAs <- apply(apply(votes_matrix,2,is.na),2,sum)
votes_matrix <- votes_matrix[,count_NAs<215]
...
sample_fit <- sampling(object=compiled_model,
                       ...
                       iter=2000, chains=2,warmup=1000,cores=2)
...
# 
legis_data <- as.data.table(legis_data,key="id")
legis_data <- merge(legis_data,party_info,by.x="id",by.y="legis.names")

legis_data$Party <- factor(legis_data$Party,levels=c("Afek Tounes, le mouvement national et l'appel des tunisiens à l'étranger",
                                                                 "Bloc Al Horra",
...
ggplot(data=legis_data,aes(y=reorder(id,stan_mean),x=stan_mean,colour=Party)) + geom_point() + my_theme + 
  geom_text(aes(label=id),check_overlap=TRUE,hjust=1.5) + 
   theme(axis.text.y=element_blank()) + ylab("") + xlab("1st Dimension: Government v. Opposition") +
  geom_errorbarh(aes(xmax=highci,xmin=lowci)) + geom_vline(xintercept=0,alpha=0.5) + scale_colour_brewer(palette="Set1")

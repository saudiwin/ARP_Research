...
if(run_model) {
  ...
  unam_votes <- group_by(all_votes, bill_id) %>% 
    summarize(unan=all(vote_choice[!is.na(vote_choice)]=="YES") || all(vote_choice[!is.na(vote_choice)]=="NO"))
  
  all_votes <- anti_join(all_votes,filter(unam_votes,unan),by="bill_id")
  ...
  bill_stat <- left_join(bill_stat,bill_stat_nahda, by=c("bill_id","vote_choice"))
  
  bill_sum <- group_by(bill_stat, bill_id) %>% 

...
parameters {
  ...
  vector[num_bills-gov_num] B_yes;
  vector[num_bills] sigma;
  //vector[opp_num] sigma_opp;
  vector<upper=0>[gov_num] B_gov;
...
model {	
  ...
  //model
  for(n in 1:N) {
      pi[n] = append_row(rep_vector(0,1),sigma[bb[n]] * ( L_open[ll[n]] - B_adj[bb[n]] - steps));
      pi[n] = cumulative_sum(pi[n]);
	    Y[n] ~ categorical_logit(pi[n]);
  }
